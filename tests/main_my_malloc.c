...
void simple_malloc(void) {
    ...
    tab[1] = my_malloc(80);
    tab[2] = my_malloc(80);
    tab[3] = my_malloc(1000);
    tab[4] = my_malloc(1000);
    tab[5] = my_malloc(1000);
    tab[6] = my_malloc(1500);
    tab[7] = my_malloc(1500);
    tab[8] = my_malloc(3900);
    tab[9] = my_malloc(3900);
    for (int i = 0; i < 10; ++i)
        free(tab[i]);
}
...
void test_realloc() {
    ...
    char *test = my_malloc(first_size + 1);
...
void test_realloc_array(void) {
    ...
    char *tab = my_malloc(first_size + 1);
...
int main() {
    block_t *arena = arena_control();
    char *tab[MALLOC_INIT_SZ * 1000] = {0};
//    char **tab = my_malloc(MALLOC_INIT_SZ);
